...
- **Share Role (member / viewer):** Setiap undangan membawa `role` yang dikirim opsional pada `POST /api/v1/wallets/:id/invites` (default `member`). `member` = baca + tulis (boleh mencatat transaksi pada dompet bersama), sedangkan `viewer` = hanya-baca (boleh melihat dompet, transaksi, report, export, dan dashboard analytics, tetapi semua jalur tulis ditolak). Role disimpan di kolom `wallet_shares.role` dan defaultnya `member` sehingga semua share lama tetap baca+tulis. Undangan yang masih *pending* (belum diterima) hanya dapat melihat bahwa dompet itu ada; saldo, daftar anggota, dan total goal-pool disembunyikan sampai undangan diterima.
