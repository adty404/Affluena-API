...
- 2 tags.
- **~7 transactions per day across the last 3 complete calendar months (~620–680 total, ~210/month)** — densely lived-in history so Kalender, Wawasan, history, and the 6/12-month cashflow-trend and net-worth views are non-flat and feel real. The seeded months are the 3 calendar months *before* the seed run (e.g. seeding on 2026-07-04 fills **April, May and June 2026**), so the history is always fully past — never future-dated. Each month is generated by the same deterministic per-day loop (`seedMonthTransactions`) keyed by a per-month offset, so amounts/notes vary a little between months (the trend line is not flat) while staying **fully deterministic** — every re-seed produces identical content (only the row UUIDs differ), idempotent by the delete-then-recreate. Per day: 3 food spends + 1 transport (+ a weekly fuel fill-up and roughly-twice-weekly grocery run) + a rotating shopping/entertainment spend, topped up to 7; fixed monthly bills land on days 3/10/15/25. Income is added on top of every month (salary on day 1 + freelance on a few days, monthly salary nudged per month), plus 3 inter-wallet topup transfers per month and 1 debt-disbursement expense overall. Dates anchor to the first of each month plus a day/time offset, built in **WIB (UTC+7)** so late-evening rows never spill into the next month for the WIB reader (`time.Date(monthStart.Year(), monthStart.Month(), day, ..., wib)`), and each row lands strictly inside its month (days clamped to the real month length). Expense mix keeps **Food & Dining the largest bucket (~55–60% by value, the majority of rows)**, then Transportation, Shopping, Entertainment, Bills; transactions spread across GoPay / Cash / OVO / BCA. Amounts are realistic IDR whole-rupiah minor units. Wallet `balance_minor` values are fixed, illustrative opening balances — the seeder does **not** recompute them from the transaction ledger (matching the long-standing seed convention), so there is no balance/ledger invariant to maintain.
- 4 category budgets for the **latest** seeded month only (the previous complete month); the two older months intentionally have no budgets.
- 1 payable debt.
- 1 subscription.
- 1 installment.
- 1 recurring rule.
...
After running `make seed`, `/api/v1/dashboard/summary` should show non-empty wallet, cashflow, budget, debt, tracker, recurring, and goal data for `demo@affluena.com`. The exact monthly values depend on the current date: transactions cover the **last 3 complete calendar months** (nothing in the current or a future month), budgets are seeded for the latest of those (the previous complete month), and the operational entities (debt/subscription/installment/recurring/goal due dates) are anchored relative to `time.Now()`. Query the previous complete month (e.g. `?month=YYYY-MM`) to see the budget-vs-actual figures; the multi-month history is what makes `GET /api/v1/dashboard/cashflow-trend?months=6` non-flat.
