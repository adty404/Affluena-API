...
- `DELETE /api/v1/auth/account` - **Permanent account deletion** (Google Play requirement) -> `{ password }` -> `204 No Content`. The password MUST be re-entered and is verified against the stored bcrypt hash, so a stolen access token alone cannot destroy an account. Errors: `400` when `password` is missing/invalid body, `401 { "error": "password is incorrect" }` when it does not match. On success, one database transaction deletes the user row and every row it owns — wallets, categories, transactions, tags, budgets, debts, installments, subscriptions, recurring rules, quick entry templates, goals, notifications, export jobs, sent alerts, api_logs, password reset tokens, and **all sessions/refresh tokens** (every device is signed out; the still-unexpired access JWT can no longer act because the user row is gone). Sharing cleanup: `wallet_share_links` (Berbagi Dompet) and `wallet_shares` rows in both directions are removed, so viewers/members immediately stop seeing the deleted user's wallets; other users' transactions recorded **on the deleted user's shared wallets** are deleted with those wallets, while other members' goal wallets for the deleted owner's goals are detached and converted to regular `cash` wallets (their money and contribution history survive). This is irreversible.
...
A share link (DB table `wallet_share_links`, columns `owner_id`/`viewer_id`/`status`) is a one-way, read-only grant at the account level: once accepted, the invited person gets `viewer` access to **all** of the owner's wallets — current and any created later. It is implemented on top of wallet sharing: accepting fans out joined `viewer` `wallet_shares` (tagged `source='link'`) for every owner wallet, and creating a new wallet auto-adds the same for each joined viewer; revoking removes only those `source='link'` shares.
...
### Transaction
- `POST /api/v1/transactions` - `{ type, amount_minor, fee_minor?, transaction_at, note, wallet_id, to_wallet_id, category_id, tag_ids }`. `transaction_at` is a full RFC3339 timestamp (date **and** time-of-day, e.g. `2024-01-01T15:04:05Z`). Any date is accepted, so transactions may be backdated or future-dated. Clients should send the local datetime normalized to UTC.
  - `fee_minor` (optional, default `0`, integer minor units, `>= 0`) is a bank admin fee for a `transfer`. It is charged to the **source** wallet on top of `amount_minor`, so on a transfer the source loses `amount_minor + fee_minor` while the destination gains only `amount_minor` — the sum of all wallet balances drops by exactly `fee_minor` (the fee is a real loss, net-worth-correct). `fee_minor` is **only** valid for `type=transfer`: a non-zero `fee_minor` on any other type returns `400` (a zero/omitted fee is fine). A negative `fee_minor` returns `400`. Editing a transfer's `fee_minor`, `amount_minor`, or wallets re-derives balances atomically; deleting a transfer refunds `amount_minor + fee_minor` to the source and removes `amount_minor` from the destination. `fee_minor` is echoed on every transaction response (list, detail, create, update).
