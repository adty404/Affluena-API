...
- Auth: register, login, refresh token, protected routes. Register also seeds onboarding defaults (8 Bahasa Indonesia categories + a "Dompet Utama" starter cash wallet) in the same transaction as the user row — the response shape is unchanged and a failed register writes nothing. `DELETE /api/v1/auth/account` permanently deletes the account (password re-entry required) in one transaction — see the notable API decisions below for the cascade details.
...
- A `transfer` may carry an optional `fee_minor` (bank admin fee, default 0, `>= 0`, transfer-only): the source wallet is charged `amount_minor + fee_minor` and the destination receives only `amount_minor`, so the sum of balances drops by exactly `fee_minor` (net-worth-correct). `fee_minor` is derived only in `BalanceDeltas` (`internal/transaction/service.go`); create/update/delete all route through `BalanceDeltas` + `applyDeltas`/`reverseDeltas`, and `Transaction.Input()` carries the stored old fee, so the update-reversal and delete-refund handle the fee with no separate code path. A non-zero `fee_minor` on a non-transfer type, or any negative `fee_minor`, must be rejected with an explicit `400` (validated in the handler, not the keyword-mapped error path). See `internal/server/transaction_fee_integration_test.go`.
...
- `internal/server/transaction_fee_integration_test.go`: proves the optional transfer `fee_minor` (bank admin fee) charges `amount+fee` to the source and `amount` to the destination atomically, that the sum-of-balances drops by exactly the fee, that editing the fee/amount/source-wallet and deleting all keep balances exact, that a non-zero fee on a non-transfer type or a negative fee returns `400`, and that an omitted fee behaves as before (`fee_minor=0`).
...
- `internal/server/account_deletion_integration_test.go`: proves `DELETE /api/v1/auth/account` wipes the user row and all owned rows (login becomes 401, sessions dead), rejects wrong (401) and missing (400) passwords without touching data, removes Berbagi Dompet visibility/link rows for viewers, and survives the two cascade-abort scenarios (goal owner with member contributions — the member's goal wallet is detached to `cash` and keeps its money; shared-wallet member transactions on the deleted owner's wallets are removed).
...
- `internal/server/tracker_payment_history_integration_test.go`: proves `GET /installments/:id/payments` and `GET /subscriptions/:id/payments` return paid rows newest-first with correct `amount_minor` + `transaction_id` linkage and note round-trip, return `payments: []` before any payment, and answer 404 without leaking data for another user's tracker id.
...
- `DELETE /api/v1/auth/account` permanently deletes the caller's account (Google Play requirement). `{ password }` is required and bcrypt-verified (wrong -> `401 "password is incorrect"`, missing -> `400`); success -> `204`. Like `PUT /auth/password` revokes sessions on credential change, account deletion kills **every** session: the users-row delete cascades to refresh_tokens and all user-owned tables. `auth.Repository.DeleteUser` runs explicit pre-delete statements inside the same transaction for what the cascade cannot handle: other members' goal wallets are detached from the deleted owner's goals and converted to `cash` (wallets.goal_id -> goals is ON DELETE CASCADE and would otherwise drag another user's wallet down and abort on their RESTRICT transactions), other users' shared-wallet rows (transactions/quick entries/recurring rules/trackers/debts) that RESTRICT-reference the deleted user's wallets are deleted first, and `sent_alerts` (no users FK) + `api_logs` (SET NULL) rows are removed explicitly. The apilog middleware's async insert for the delete request itself fails harmlessly on the users FK after the commit (error is swallowed; the 204 already went out).
...
- `GET /api/v1/installments/:id/payments` and `GET /api/v1/subscriptions/:id/payments` return `{ "payments": [ { id, installment_id|subscription_id, amount_minor, paid_at, transaction_id, note } ] }` ordered `paid_at DESC` (capped at 200 rows, no pagination). The pay endpoints write these history rows (`installment_payments`/`subscription_payments`, migration `000033`) in the same transaction as the expense transaction; `note` stores the raw pay-request note (default `""`). The parent tracker is scoped by `user_id` like `GET /installments/:id`, so a foreign id returns 404.
...
- Current later migrations include financial goals (`000007`), tags (`000008`), category hierarchy (`000009`), transaction tag ownership (`000010`), category parent ownership/type checks (`000011`), shared-wallet compatibility migrations through debt records (`000016`-`000019`), sent alerts (`000020`), wallet display metadata (`000021`), user profile fields (`000022`), password reset tokens (`000023`), export jobs (`000024`), notification rules (`000025`), the shared-wallet `role` column (`000026`, `wallet_shares.role` ∈ {`member`,`viewer`} default `member`), account-level partner links (`000027`-`000028`), wallet icon (`000029`), entity colors/icons (`000030`), category icon/color plus user-arrangeable `position` with per-user backfill (`000031`), notification deliveries (`000032`), installment/subscription payment history (`000033`, `installment_payments` + `subscription_payments`; `transaction_id` is ON DELETE CASCADE — deleting the linked transaction removes the history row instead of blocking the delete), and the transfer admin fee (`000034`, `transactions.fee_minor bigint NOT NULL DEFAULT 0 CHECK (fee_minor >= 0)` — additive, safe on populated data since the default 0 preserves existing transfer balances).
