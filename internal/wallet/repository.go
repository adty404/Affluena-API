...
func (r *Repository) List(ctx context.Context, userID string, pagination page.Params) (page.Result[Wallet], error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id::text, user_id::text, name, type, currency_code, balance_minor, goal_id::text, created_at, updated_at
		FROM wallets
		WHERE user_id = $1
	...
	for rows.Next() {
		var wallet Wallet
		if err := rows.Scan(&wallet.ID, &wallet.UserID, &wallet.Name, &wallet.Type, &wallet.CurrencyCode, &wallet.BalanceMinor, &wallet.GoalID, &wallet.CreatedAt, &wallet.UpdatedAt); err != nil {
			return page.Result[Wallet]{}, err
		}
...
func (r *Repository) Get(ctx context.Context, userID string, id string) (Wallet, error) {
	...
	`, userID, id).Scan(&wallet.ID, &wallet.UserID, &wallet.Name, &wallet.Type, &wallet.CurrencyCode, &wallet.BalanceMinor, &wallet.GoalID, &wallet.CreatedAt, &wallet.UpdatedAt)
