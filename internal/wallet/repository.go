...
func (r *Repository) Create(ctx context.Context, userID string, input CreateWalletInput) (Wallet, error) {
	...
	err := r.pool.QueryRow(ctx, `
		INSERT INTO wallets (user_id, name, type, currency_code, balance_minor, color, icon, description, goal_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id::text, user_id::text, name, type, currency_code, balance_minor, color, icon, description, goal_id::text, created_at, updated_at
	`, userID, input.Name, input.Type, input.CurrencyCode, input.BalanceMinor, input.Color, input.Icon, input.Description, input.GoalID).Scan(
		&wallet.ID, &wallet.UserID, &wallet.Name, &wallet.Type, &wallet.CurrencyCode, &wallet.BalanceMinor, &wallet.Color, &wallet.Icon, &wallet.Description, &wallet.GoalID, &wallet.CreatedAt, &wallet.UpdatedAt,
	)
	return wallet, err
}
...
func (r *Repository) List(ctx context.Context, userID string, pagination page.Params) (page.Result[Wallet], error) {
	rows, err := r.pool.Query(ctx, `
		SELECT DISTINCT w.id::text, w.user_id::text, w.name, w.type, w.currency_code, w.balance_minor, w.color, w.icon, w.description, w.goal_id::text, w.created_at, w.updated_at,
	...
	for rows.Next() {
		var wallet Wallet
		if err := rows.Scan(&wallet.ID, &wallet.UserID, &wallet.Name, &wallet.Type, &wallet.CurrencyCode, &wallet.BalanceMinor, &wallet.Color, &wallet.Icon, &wallet.Description, &wallet.GoalID, &wallet.CreatedAt, &wallet.UpdatedAt, &wallet.Role, &wallet.ShareStatus); err != nil {
			return page.Result[Wallet]{}, err
		}
...
func (r *Repository) Get(ctx context.Context, userID string, id string) (Wallet, error) {
	...
	`, userID, id).Scan(&wallet.ID, &wallet.UserID, &wallet.Name, &wallet.Type, &wallet.CurrencyCode, &wallet.BalanceMinor, &wallet.Color, &wallet.Icon, &wallet.Description, &wallet.GoalID, &wallet.CreatedAt, &wallet.UpdatedAt, &wallet.Role, &wallet.ShareStatus)
...
func (r *Repository) Update(ctx context.Context, userID string, id string, input UpdateWalletInput) (Wallet, error) {
	...
	err := r.pool.QueryRow(ctx, `
		UPDATE wallets
		SET name = $3, type = $4, currency_code = $5, color = $6, icon = $7, description = $8, updated_at = now()
		WHERE user_id = $1 AND id = $2
		RETURNING id::text, user_id::text, name, type, currency_code, balance_minor, color, icon, description, goal_id::text, created_at, updated_at
	`, userID, id, input.Name, input.Type, input.CurrencyCode, input.Color, input.Icon, input.Description).Scan(
		&wallet.ID, &wallet.UserID, &wallet.Name, &wallet.Type, &wallet.CurrencyCode, &wallet.BalanceMinor, &wallet.Color, &wallet.Icon, &wallet.Description, &wallet.GoalID, &wallet.CreatedAt, &wallet.UpdatedAt,
	)
	if errors.Is(err, pgx.ErrNoRows) {
		return Wallet{}, ErrNotFound
...
// AddPartnerViewerShares grants every joined account-level viewer of the owner
// a viewer share on the given wallet (used when a new wallet is created so the
// "a viewer sees all my wallets" invariant holds). Tagged source='link' so it
// is cleaned up if the share link is later revoked; never clobbers a manual
// share.
func (r *Repository) AddPartnerViewerShares(ctx context.Context, ownerID string, walletID string) error {
	_, err := r.pool.Exec(ctx, `
		...
		SELECT $1, pl.viewer_id, 'joined', 'viewer', 'link'
